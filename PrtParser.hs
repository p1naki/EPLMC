...
data HappyAbsSyn t4
	= HappyTerminal (PtToken)
	| HappyErrorToken Int
	| HappyAbsSyn4 t4

happyExpList :: Happy_Data_Array.Array Int Int
happyExpList = Happy_Data_Array.listArray (0,54) ([3632,512,227,7,0,0,0,3632,1136,49152,12344,14,7168,14528,256,96,49152,56,1,4096,1536,0,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_prtexpr","PrtExpr","ATOM","FORM","'+'","'.'","'*'","'@'","'$'","'('","')'","%eof"]
        bit_start = st * 14
        bit_end = (st + 1) * 14
...
action_9 (8) = happyShift action_19
action_9 (9) = happyShift action_13
action_9 (13) = happyShift action_20
...
action_14 (8) = happyShift action_12
...
happyReduction_7 (HappyTerminal (PtTokenTest happy_var_1))
	 =  HappyAbsSyn4
		 (PtTest happy_var_1
	)
...
happyReduction_8 _
	(HappyTerminal (PtTokenTest happy_var_2))
	_
	 =  HappyAbsSyn4
		 (PtTest happy_var_2
	)
...
happyReduction_9 (HappyTerminal (PtTokenSym happy_var_1))
	 =  HappyAbsSyn4
		 (PtLit happy_var_1
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	PtTokenSym happy_dollar_dollar -> cont 5;
	PtTokenTest happy_dollar_dollar -> cont 6;
	PtTokenAlt -> cont 7;
	PtTokenCat -> cont 8;
	PtTokenStar -> cont 9;
	PtTokenDel -> cont 10;
	PtTokenEps -> cont 11;
	PtTokenLParen -> cont 12;
	PtTokenRParen -> cont 13;
	_ -> happyError' ((tk:tks), [])
	}
...
happyError' :: () => ([(PtToken)], [String]) -> HappyIdentity a
happyError' = HappyIdentity . (\(tokens, _) -> parsePtErr tokens)
prtexpr tks = happyRunIdentity happySomeParser where
