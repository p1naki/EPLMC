...
data HappyAbsSyn t4
	= HappyTerminal (BFToken)
	| HappyErrorToken Int
	| HappyAbsSyn4 t4

happyExpList :: Happy_Data_Array.Array Int Int
happyExpList = Happy_Data_Array.listArray (0,38) ([400,25608,192,51200,1600,36876,1,8292,1539,32968,0,33280,32780,0,34,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_boolform","BForm","ATOM","'&'","'|'","'~'","'('","')'","%eof"]
        bit_start = st * 11
        bit_end = (st + 1) * 11
...
action_7 (6) = happyShift action_17
...
action_20 (6) = happyShift action_10
...
happyReduction_7 (HappyTerminal (BFTokenSym happy_var_1))
	 =  HappyAbsSyn4
		 (PrpA happy_var_1
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	BFTokenSym happy_dollar_dollar -> cont 5;
	BFTokenAnd -> cont 6;
	BFTokenOr -> cont 7;
	BFTokenNeg -> cont 8;
	BFTokenLParen -> cont 9;
	BFTokenRParen -> cont 10;
	_ -> happyError' ((tk:tks), [])
	}
...
happyError' :: () => ([(BFToken)], [String]) -> HappyIdentity a
happyError' = HappyIdentity . (\(tokens, _) -> parseBFError tokens)
boolform tks = happyRunIdentity happySomeParser where
