...
data HappyAbsSyn t4
	= HappyTerminal (ObToken)
	| HappyErrorToken Int
	| HappyAbsSyn4 t4

happyExpList :: Happy_Data_Array.Array Int Int
happyExpList = Happy_Data_Array.listArray (0,45) ([1808,16512,28700,0,0,32768,57400,57864,7232,0,8206,8206,1536,1808,16,16384,3072,0,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_obexpr","ObExpr","ATOM","'+'","'.'","'*'","'@'","'$'","'('","')'","%eof"]
        bit_start = st * 13
        bit_end = (st + 1) * 13
...
action_3 (7) = happyShift action_10
...
action_8 (7) = happyShift action_16
...
happyReduction_7 (HappyTerminal (ObTokenSym happy_var_1))
	 =  HappyAbsSyn4
		 (ObLit happy_var_1
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	ObTokenSym happy_dollar_dollar -> cont 5;
	ObTokenAlt -> cont 6;
	ObTokenCat -> cont 7;
	ObTokenStar -> cont 8;
	ObTokenDel -> cont 9;
	ObTokenEps -> cont 10;
	ObTokenLParen -> cont 11;
	ObTokenRParen -> cont 12;
	_ -> happyError' ((tk:tks), [])
	}
...
happyError' :: () => ([(ObToken)], [String]) -> HappyIdentity a
happyError' = HappyIdentity . (\(tokens, _) -> parseObErr tokens)
obexpr tks = happyRunIdentity happySomeParser where
